...
function doExportPDF() {
  ...
  var svgString = currentRenderer.exportSVG();
  if (!svgString) return;
  var stats = document.getElementById('global-stats-bar');
  var statsHTML = stats ? stats.innerHTML : '';
  var win = window.open('', '_blank');
  win.document.write('<!DOCTYPE html><html><head><title>net.sketch — Model Architecture</title>');
  win.document.write('<link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700&display=swap" rel="stylesheet">');
  win.document.write('<style>');
  win.document.write('* { margin: 0; padding: 0; box-sizing: border-box; }');
  win.document.write("body { font-family: 'Inter', 'Helvetica Neue', Arial, sans-serif; background: #fff; padding: 20px; }");
  win.document.write('.header { text-align: center; margin-bottom: 16px; }');
  win.document.write('.header h1 { font-size: 14px; font-weight: 600; color: #333; letter-spacing: -0.3px; }');
  win.document.write('.header h1 span { font-weight: 400; background: linear-gradient(135deg, #2997ff, #f9b248); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }');
  win.document.write('.stats { display: flex; justify-content: center; gap: 24px; margin-bottom: 16px; font-size: 11px; color: #666; }');
  win.document.write('.stats b { color: #333; font-size: 13px; }');
  win.document.write('.graph { text-align: center; }');
  win.document.write('.graph svg { max-width: 100%; height: auto; }');
  win.document.write('.footer { text-align: center; margin-top: 16px; font-size: 9px; color: #aaa; }');
  win.document.write('@media print { body { padding: 10px; } @page { margin: 10mm; size: landscape; } }');
  win.document.write('</style></head><body>');
  win.document.write('<div class="header"><h1>net<span>.sketch</span></h1></div>');
  if (statsHTML) win.document.write('<div class="stats">' + statsHTML + '</div>');
  win.document.write('<div class="graph">' + svgString + '</div>');
  win.document.write('<div class="footer">Generated by net.sketch.io</div>');
