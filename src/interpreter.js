...
export class Interpreter {
  ...
  applyLayer(layer, args, kwargs, env) {
    const inputs = args.filter(a => a?.kind === 'tensor');
  ...
  estimateParams(type, config, inputShapes, outputShape) {
    ...
    switch (type) {
      case 'Conv2D': case 'Conv1D': case 'Conv3D': {
        ...
        return typeof inC === 'number' ? (Math.pow(k, dim) * inC + 1) * f : 0;
      }
      case 'Conv2DTranspose': { const k = this.normalizeInt(config.kernel_size, 3); const f = config.filters || 0; return typeof inC === 'number' ? (k * k * inC + 1) * f : 0; }
      case 'SeparableConv2D': { const k = this.normalizeInt(config.kernel_size, 3); const f = config.filters || 0; return typeof inC === 'number' ? k * k * inC + (inC + 1) * f : 0; }
      case 'DepthwiseConv2D': { const k = this.normalizeInt(config.kernel_size, 3); return typeof inC === 'number' ? (k * k + 1) * inC : 0; }
      case 'Dense': { const u = config.units || 0; return typeof inC === 'number' ? (inC + 1) * u : 0; }
