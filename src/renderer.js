...
export class GraphRenderer {
  ...
  nodeDims(node, maxRes, maxCh) {
    ...
    if (node.type === 'Input' || node.type === 'Output') {
      ...
      return { w: Math.round(Math.max(30, 20 + Math.sqrt(chFrac) * 120)), h: Math.round(20 + resFrac * 55) };
    }
  ...
  drawNode(parent, node, pos) {
    ...
    if (isDown || isUp || isResize) {
      ...
      const ioLbl = this.el('text', { x: w / 2, y: -2, 'text-anchor': 'middle', fill: c.text, 'font-size': '11px', 'font-style': 'italic', 'font-family': 'serif' });
      ioLbl.textContent = node.type === 'Input' ? 'x' : 'y';
      g.appendChild(ioLbl);
      const s = node.outputShape || [];
      const sh = (s.length >= 4 && typeof s[1] === 'number') ? s[1] : null;
      const sw = (s.length >= 4 && typeof s[2] === 'number') ? s[2] : null;
      const sc = s.length >= 2 ? s[s.length - 1] : null;
      const dd = w * 0.22;
      const cw2 = w - dd - 2, ch2 = h * 0.7;
      const bx = 1, by = (h - ch2 - dd) / 2 + dd;
      g.appendChild(this.el('path', { d: `M${bx},${by} L${bx+cw2},${by} L${bx+cw2},${by+ch2} L${bx},${by+ch2} Z`, fill: c.stroke, opacity: 0.2, stroke: c.stroke, 'stroke-width': 0.7 }));
      g.appendChild(this.el('path', { d: `M${bx},${by} L${bx+dd},${by-dd} L${bx+cw2+dd},${by-dd} L${bx+cw2},${by} Z`, fill: c.stroke, opacity: 0.1, stroke: c.stroke, 'stroke-width': 0.5 }));
      g.appendChild(this.el('path', { d: `M${bx+cw2},${by} L${bx+cw2+dd},${by-dd} L${bx+cw2+dd},${by+ch2-dd} L${bx+cw2},${by+ch2} Z`, fill: c.stroke, opacity: 0.14, stroke: c.stroke, 'stroke-width': 0.5 }));
      if (sh !== null) {
        const hLbl = this.el('text', { x: bx - 2, y: by + ch2 / 2, 'text-anchor': 'end', 'dominant-baseline': 'central', fill: c.text, 'font-size': '5px', 'font-family': font, opacity: 0.7 });
        hLbl.textContent = sh;
        g.appendChild(hLbl);
      }
      if (sw !== null) {
        const wLbl = this.el('text', { x: bx + cw2 / 2, y: by + ch2 + 7, 'text-anchor': 'middle', fill: c.text, 'font-size': '5px', 'font-family': font, opacity: 0.7 });
        wLbl.textContent = sw;
        g.appendChild(wLbl);
      }
      if (sc !== null) {
        const mx = bx + cw2 + dd / 2, my = by - dd / 2;
        const cLbl = this.el('text', { x: mx + 3, y: my, 'text-anchor': 'start', 'dominant-baseline': 'central', fill: c.text, 'font-size': '5px', 'font-family': font, opacity: 0.7 });
        cLbl.textContent = sc;
        g.appendChild(cLbl);
      }
    } else {
  ...
  drawEdgeDagre(parent, edge, graph, positions) {
    ...
    if (isToCircle) {
      const cx = to.x + to.w / 2, cy = to.y + to.h / 2, r = Math.min(to.w, to.h) / 2;
      const prev = allPts[allPts.length - 2];
      const dx = cx - prev.x, dy = cy - prev.y;
      const dist = Math.sqrt(dx * dx + dy * dy) || 1;
      x2 = cx - (dx / dist) * r;
      y2 = cy - (dy / dist) * r;
      allPts[allPts.length - 1] = { x: x2, y: y2 };
    }
